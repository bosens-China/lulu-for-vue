...
点击页面顶部的“主题调色盘”按钮，在任意文档页面分别调整浅色和深色配色。调整强调色会同步主按钮底色，也可以单独微调按钮。

配色和编辑模式自动保存在当前浏览器，刷新或重新打开后恢复；“恢复当前主题默认值”只重置正在编辑的一套配色。预览只作用于调色盘内的组件区域；复制生成的 CSS 后，放在 LuLu 样式之后即可应用到项目。浏览器禁用存储时仍可预览和导出。
