<template>
  <main class="home-page relative flex-1 overflow-hidden bg-[var(--lulu-color-bg-page)]">
    <div class="pointer-events-none absolute inset-x-0 top-0 h-112 overflow-hidden" aria-hidden="true">
      <div class="absolute -left-24 top-10 h-72 w-72 rounded-full bg-[#1677ff]/12 blur-3xl" />
      <div class="absolute -right-16 top-28 h-80 w-80 rounded-full bg-[#22d3ee]/10 blur-3xl" />
    </div>
    ...
    <section class="relative mx-auto grid min-h-[calc(100vh-4rem)] max-w-7xl items-center gap-14 px-6 py-16 lg:grid-cols-[1.05fr_0.95fr] lg:py-24">
      <div class="max-w-2xl">
        ...
        <h1 class="m-0 text-5xl font-extrabold leading-[1.08] tracking-tight text-[var(--lulu-color-text-heading)] sm:text-6xl lg:text-7xl">
          简洁、可靠的
          <span class="block bg-gradient-to-r from-[var(--lulu-color-primary)] to-[#06b6d4] bg-clip-text text-transparent">
            Vue 组件库
          </span>
        </h1>
        ...
        <div class="mt-9 flex flex-wrap items-center gap-3">
          <a
            ...
            class="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-[var(--lulu-color-primary-solid)] px-5 text-sm font-semibold text-[var(--lulu-color-text-inverse)] shadow-[0_8px_24px_rgba(22,119,255,0.24)] text-decoration-none transition-all duration-200 hover:-translate-y-0.5 hover:brightness-105"
          >
            浏览全部组件
      ...
      <div class="relative mx-auto w-full max-w-lg lg:mx-0" aria-hidden="true">
        <div class="absolute -inset-6 rounded-[2.5rem] bg-gradient-to-br from-[#1677ff]/14 to-[#22d3ee]/5 blur-2xl" aria-hidden="true" />
        <div class="relative overflow-hidden rounded-3xl border border-solid border-[var(--lulu-color-border-subtle)] bg-[var(--lulu-color-bg-container)]/92 p-5 shadow-[0_28px_80px_rgba(15,23,42,0.14)] backdrop-blur sm:p-7">
          ...
          <div class="rounded-2xl border border-solid border-[var(--lulu-color-border-subtle)] bg-[var(--lulu-color-bg-page)] p-5 sm:p-6">
            ...
            <div class="flex h-10 items-center rounded-lg border border-solid border-[var(--lulu-color-primary)] bg-[var(--lulu-color-bg-container)] px-3 text-sm text-[var(--lulu-color-text-heading)] shadow-[0_0_0_2px_rgba(22,119,255,0.1)]">
              LuLu Design
            </div>
...
const features = [
  { mark: '37', title: '丰富的组件', description: '覆盖表单、反馈、导航与布局等常见产品场景。' },
