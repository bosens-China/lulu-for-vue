...
- 顶部提供亮色、深色切换与主色调选择。主色选择立即作用于整个文档站，按模式自动调整明度；背景和文字保留各模式默认配色，选择保存在当前浏览器，可恢复默认主色。
...
- 手机导航打开后聚焦标题；键盘聚焦时显示主题色提示，关闭后还原触发按钮焦点。手机搜索框采用至少 16px 字号，避免 iOS 输入聚焦时页面自动放大；侧栏折叠按钮保留可见键盘焦点。
...
- 弹窗打开时焦点进入内部，普通弹窗默认聚焦标题并允许原生 `autofocus` 覆盖；命令式确认优先聚焦取消按钮。键盘聚焦静态标题时显示主题化焦点提示，关闭后焦点返回触发元素。
