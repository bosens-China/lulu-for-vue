...
- 导航区分输入选择、日期时间与表单校验，组合组件相邻排列。每个组件页提供实际样式使用的 CSS Tokens、用途、明暗默认值及覆盖方式；示例覆盖有意义的属性状态、边界和自定义插槽，场景入口见[文档走查](./docs-experience-audit.md)。
- 组合组件复用完整场景：Tab/TabPanel 链接至 Tabs，浮层共用位置语义说明。所有 Demo 使用公开入口且可查看源码；表单复位同步业务模型与校验错误，消息示例展示自动关闭和手动清理。
