...
## 2026-09-26 布局、焦点与手机搜索复核

在本地真实浏览器逐页访问首页、4 个指南和 37 个组件页，共 42 个路由、80 组 Demo；分别用桌面与 390×844 手机视口检查标题、静态示例预览和页面宽度。未发现页面级横向溢出或静态预览控件重叠。分页与标签项间距为 4px，属于组件现有紧凑布局。另在 320px 宽度实测 DialogHost 操作行自动换行、确认弹窗的两个底部按钮相隔 12px；该次检查没有打开自定义内容弹窗。

实测 Dialog 基础示例打开后聚焦标题，DialogHost 确认弹窗聚焦取消按钮，Escape 关闭后焦点返回触发按钮。手机组件导航原先把焦点放在额外的 `autofocus` 标题元素上，并用 `outline-none` 隐藏提示；现在直接使用 Dialog 的标题和焦点样式。键盘打开时显示主题色焦点提示，鼠标打开时不显示。桌面侧栏折叠按钮也补回键盘焦点提示。

手机搜索框原计算字号为 14px，虽在当前 Chromium 手机视口未出现缩放，但该字号会触发 iOS Safari 聚焦输入框时的自动放大。现仅在手机宽度下将文档搜索框设为 16px；浏览器复验聚焦后字号为 16px、搜索框仍正常获得焦点。iOS Safari 真机缩放行为尚需在对应设备上最终确认。

后续用户提供线上 iPhone 截图，指出上次未打开的 DialogHost“自定义弹窗”内容按钮贴合。线上浏览器复现“增加 / 完成”按钮间距为 0px；示例的动态内容直接并排渲染两个按钮。已改用示例原本引入的 `LuluSpace` 包裹操作按钮。修正后在 390px 和 320px 视口实测间距均为 12px，计数更新、关闭与焦点归还正常。此处说明前述逐页扫描只覆盖静态预览布局，没有覆盖所有动态弹窗内容。

提交前进一步检查了全部 81 个示例 Vue 文件，包括 DialogHost 的 `DialogActions.vue`，重点核对渲染函数、操作按钮容器与窄屏排列。MessageHost 渲染函数已有换行与间距；分页边界、表单重置和按钮重置的操作行补 `flex-wrap`，避免窄屏时把按钮挤入单行。320px 浏览器复验三组操作行均无页面级溢出，长文案按钮正常换行。
